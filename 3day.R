...
#
#2.2factor(범주형,categoriacal)
#
#factor형:문자형태로 저장되며 범위를 갖는 값을 저장
#         하는 data type
#factor():펙터형 벡터 생성 함수
bt <- c('A','B','B','O','AB','A')   #문자형 벡터
bt.factor <- factor(bt)             #팩터형 벡터

gender <- c('남','여','여','남','여','여','남')
...
z <- matrix(1:20,nrow=4,ncol=5,byrow=T)-행우선
z
...
#
colSums(iris[ ,-5]);apply(iris[ ,4:4],2,sum)
...
#csv file내용 읽어서 data frame생성

air <- read.csv('C:\\Workspace\\workspace R\\',header=T)
air2 <- read.csv('C:\\Workspace\\workspace R',header=F)
